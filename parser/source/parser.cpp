...
std::string Parser::ParseExpression(const std::string& expression) {
    if(false == this->IsExpressionValid(expression)) return "";

    std::string output_expression;
    std::stack<char> operations;

    auto add_operation = [](const std::string& str, const char& operation) {
    ...
    for(auto iter = expression.begin(); iter < expression.end() + 1; iter++) {
        if(iter == expression.end()) {
            ...
            output_expression = add_operation(output_expression, operations.top());
            operations.pop();
            iter--;
            continue;
        }
        ...
        switch(*iter) {
            ...
            case '^': {
                ...
                auto new_operation_priority = operations_priorities_.find( *iter )->second;
                auto last_operation_priority = operations_priorities_.find( operations.top() )->second;

                if( new_operation_priority > last_operation_priority ) {
                    operations.push(*iter);
                } else if( new_operation_priority <= last_operation_priority ) {
                    output_expression = add_operation(output_expression, operations.top());
                    operations.pop();
                    iter--;
                }
...
bool Parser::IsOperandsValid(const std::string& expression) const {
    for(auto iter = expression.begin(); iter < expression.end(); iter++) {
        if( helper_.IsDigit(iter) ) {
            ...
            if( *operand.begin() == '.' || *operand.end() == '.' ) return false;
            ...
            if( idx != std::string::npos ) {
                ...
                if( operand.find('.') != std::string::npos ) return false;
...
bool Parser::IsOperatorsValid(const std::string& expression) const {
    if( helper_.IsOperator(expression.begin()) || helper_.IsOperator(expression.end()) ) return false;
    ...
    for(auto iter = expression.begin(); iter < expression.end(); iter++) {
        if( helper_.IsOperator(iter) ) {
            ...
            if( helper_.IsOperator(iter + 1) ) return false;

            operators_count++;
        }
    }

    if( operators_count >= operands_count || operators_count < operands_count - 1 ) return false;

    return true;
}

bool Parser::IsBracketsValid(const std::string& expression) const {
    if( *expression.begin() == ')' || *expression.end() == '(' ) return false;

    if( expression.find('(') == std::string::npos && expression.find(')') == std::string::npos ) return true;
